...
% Perfis: perfil(Trilha, Caracteristica, Peso) - 1-5.
...
% Perguntas: pergunta(ID, Texto, Caracteristica) - Textos casuais, ajustados pro novo nome.
...
:- dynamic(resposta/2).  % Armazena respostas em runtime.

% Resposta por característica: Pega via ID, default 'n'.
resposta_por_caracteristica(Caracteristica, Resposta) :-
    pergunta(ID, _, Caracteristica),
    (resposta(ID, R) -> Resposta = R ; Resposta = n).

% Calcula pontuação: findall coleta TODOS pesos de 's', soma (0 se vazio).
...
% Recomenda: findall para TODAS trilhas, sort decrescente, cut para única solução.
recomenda(ListaRecomendacoes) :-
    ...
    remove_pontuacao(ListaOrdenada, ListaRecomendacoes), !.  % Cut para evitar backtracking.
...
% Inicia: Checa se já tem respostas ANTES de limpar (preserva testes).
iniciar :-
    (tem_respostas ->
        % Se já tem, pula limpeza e perguntas.
        true
    ;   % Senão, limpa e pergunta.
        retractall(resposta(_, _)),
        faz_perguntas
    ),
    recomenda(Lista), !,  % Cut para única solução.
    exibe_resultado(Lista).

tem_respostas :- resposta(_, _).

% Faz perguntas: Recursão por ID não respondido, valida input.
faz_perguntas :-
    pergunta(ID, Texto, _),
    \+ resposta(ID, _),
    format('~w (s/n): ', [Texto]),
...
% Exibe resultado: Ranking numerado com scores e just.
exibe_resultado([]) :-
...
% Justifica: findall coleta TODOS chars com 's' para a trilha específica.
justifica_recomendacao(Trilha) :-
    findall(Char, 
...
% Novo: Modo teste dedicado (carrega + recomenda + exibe, sem prompts).
testar(N) :-
    carrega_teste(N),
    recomenda(Lista), !,
    exibe_resultado(Lista).
...
% Teste 1: Lógico/dados (IA e Dados).
carrega_teste(1) :-
    retractall(resposta(_, _)),
...
% Teste 2: Redes/infra (Redes e Segurança top, com resolucao_de_problemas).
carrega_teste(2) :-
    retractall(resposta(_, _)),
...
% Teste 3: Web/criativo (Web top).
carrega_teste(3) :-
    retractall(resposta(_, _)),
    assertz(resposta(3, s)),  % programacao
    assertz(resposta(4, s)),  % design_visual
    assertz(resposta(9, s)),  % criatividade
    writeln('-- Teste 3 carregado: Foco em web e criatividade.').

% Novo Debug: Lista todas respostas assertadas.
verifica_respostas :-
    findall(ID-R, resposta(ID, R), List),
    format('Respostas assertadas: ~w~n', [List]).

% Novo Debug: Imprime pontuações para todas trilhas.
verifica_pontuacao :-
